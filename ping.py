...
class Ping:
    def ping(self, dest_name, packet_count=10):
        f = os.popen('/bin/ping %s -c %d' % (dest_name, packet_count))
        result = {}
        for line in f.readlines():
            ...
            if m:
                ...
                logging.getLogger().debug("packet loss: %s" % (m.group(1)))

            m2 = re.search("rtt min/avg/max/mdev = [^\/]+/([^\/]+)/[^\/]+/[^ ]+ ms", line)
            if m2:
                result['packet_latency'] = m2.group(1)
                logging.getLogger().debug("packet latency: %s" % (m2.group(1)))
...
if __name__ == "__main__":
    ...
    traceRoute = Ping()
    result = traceRoute.ping(dest_name=opts.dest_host)
    ...
    if not opts.raw_run:
        ...
        if result.get("packet_loss"):
            key = opts.zabbix_key + ".packet_loss"
            val = result.get('packet_loss')
            logger.info("send data to zabbix_server:%s,zabbix_host:%s item=%s  value=%s" % (opts.zabbix_server, opts.zabbix_host, key,val))
            respond = sender.send(opts.zabbix_host, key, val)
            logger.info(respond)

        if result.get("packet_latency"):
            key = opts.zabbix_key + ".packet_latency"
            val = result.get('packet_latency')
            logger.info("send data to zabbix_server:%s,zabbix_host:%s item=%s  value=%s" % (opts.zabbix_server, opts.zabbix_host, key,val))
            respond = sender.send(opts.zabbix_host, key, val)
            logger.info(respond)
